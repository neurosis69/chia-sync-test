...
class Blockchain(BlockchainInterface):
    ...
    async def create(
        ...
        num_workers = max({{ BLOCKCHAIN_CONSENSUS_THREADS }}, 1)
    ...
    async def receive_block(
        ...
        if round(transaction_end - transaction_start,2) >= 0.01:
            log.info(f"TEST Time to write block {block.height} to database: {round(transaction_end - transaction_start,2):0.2f} seconds")
