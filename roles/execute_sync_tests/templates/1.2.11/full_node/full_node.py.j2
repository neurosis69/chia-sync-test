...
class FullNode:
    ...
    async def _start(self):
        ...
        {{ FULL_NODE_SQLITE_PRAGMA_JOURNAL_MODE }}
        {{ FULL_NODE_SQLITE_PRAGMA_LOCKING_MODE }}
        {{ FULL_NODE_SQLITE_PRAGMA_CACHE_SPILL }}
        {{ FULL_NODE_SQLITE_PRAGMA_PAGE_SIZE }}
        {{ FULL_NODE_SQLITE_PRAGMA_SYNCHRONOUS }}
        {{ FULL_NODE_SQLITE_PRAGMA_MMAP_SIZE }}
        {{ FULL_NODE_SQLITE_PRAGMA_CACHE_SIZE }}
        {{ FULL_NODE_SQLITE_PRAGMA_UNCOMMITTED }}
        {{ FULL_NODE_SQLITE_PRAGMA_WAL_CHECKPOINT }}
                ...
                async with self._blockchain_lock_high_priority:
                    ...
                    if advanced_peak:
                        ...
                        try:
                            ...
                            self.log.info(f"{{ RUNID }} Added blocks {height}-{end_height}")
    ...
    async def sync_from_fork_point(
        ...
        async def validate_block_batches(batch_queue):
            ...
            while True:
                ...
                self.log.info(f"{{ RUNID }} Added blocks {start_height} to {end_height}")
    ...
    async def receive_block_batch(
        ...
        if pre_validate_end - pre_validate_start > 10:
            self.log.warning(f"TEST Block pre-validation time: {pre_validate_end - pre_validate_start:0.2f} seconds")
        else:
            self.log.info(f"TEST Block pre-validation time: {pre_validate_end - pre_validate_start:0.2f} seconds")
        ...
        # TEST
        self.log.info(f"TEST Block pre-validation check time: {time.time() - pre_validate_end:0.2f} seconds")
        ...
        for i, block in enumerate(blocks_to_validate):
            ...
            receive_block_end = time.time()
            # TEST
            if round(receive_block_end - receive_block_start,2) >= 0.01:
                self.log.info(f"TEST Receive time for block {i}: {round(receive_block_end - receive_block_start,2):0.2f} seconds")
            update_all_changes_start = time.time()
            # Update all changes
            ...
            update_all_changes_end = time.time()
            # TEST
            if round(update_all_changes_end - update_all_changes_start,2) >= 0.01:
                self.log.info(f"TEST Update all changes for block {i}: {round(update_all_changes_end- update_all_changes_start,2):0.2f} seconds")
            receive_block_result_start = time.time()
            if result == ReceiveBlockResult.NEW_PEAK:
                ...
                return False, advanced_peak, fork_height, ([], {})
            receive_block_result_end = time.time()
            # TEST
            if round(receive_block_result_end - receive_block_result_start,2) >= 0.01:
                self.log.info(f"TEST Receive block result for block {i}: {round(receive_block_result_end - receive_block_result_start,2):0.2f} seconds")
            write_block_record_start = time.time()
            block_record = self.blockchain.block_record(block.header_hash)
            if block_record.sub_epoch_summary_included is not None:
                if self.weight_proof_handler is not None:
                    await self.weight_proof_handler.create_prev_sub_epoch_segments()
            # TEST
            write_block_record_end = time.time()
            if round(write_block_record_end - write_block_record_start,2) >= 0.01:
                self.log.info(f"TEST Write block record for block {i}: {round(write_block_record_end - write_block_record_start,2):0.2f} seconds")
            if round(write_block_record_end - receive_block_start,2) >= 0.01:
                self.log.info(f"TEST Total loop time for block {i}: {write_block_record_end - receive_block_start:0.2f} seconds")
        if advanced_peak:
            self._state_changed("new_peak")
            self.log.info(f"TEST Validate Time for {len(blocks_to_validate)} blocks: {time.time() - validate_start}")
            self.log.info(f"TEST Total time for {len(blocks_to_validate)} blocks: {time.time() - pre_validate_start}, "
